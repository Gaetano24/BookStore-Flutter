...
import 'package:bookstore/model/objects/authentication_data.dart';
import 'package:bookstore/model/supports/constants.dart';
import 'package:bookstore/model/supports/login_result.dart';
...
class Model {
  ...
  Future<List<Book>> getBooks({int pageNumber = 0, int pageSize = 10, String sortBy = 'id',}) async {
    final Uri uri = Uri.parse('${Constants
        .addressStoreServer}/books?pageNumber=$pageNumber&pageSize=$pageSize&sortBy=$sortBy');
    try {
  ...
  Future<List<Book>> getBooksByTitle(String title, {int pageNumber = 0, int pageSize = 10, String sortBy = 'id'}) async {
    final Uri uri = Uri.parse('${Constants.addressStoreServer}/books/title?title=$title&pageNumber=$pageNumber&pageSize=$pageSize&sortBy=$sortBy');
    try {
      final response = await http.get(uri);
      if (response.statusCode == 200) {
        List<dynamic> data = json.decode(response.body);
  ...
  Future<List<Book>> advancedSearch({
    ...
    final Uri uri = Uri.parse(
        '${Constants.addressStoreServer}/books/advancedSearch').replace(
